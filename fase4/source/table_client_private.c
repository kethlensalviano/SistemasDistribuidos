...
int test_input(int argc, char** argv){
    if (argc != 2){
        printf("Uso: ./table_client <ip_zookeeper>:<porto_zookeeper>\n");
...
int get_opcode(char* operacao){
    ...
    if(strcmp(operacao, "size") == 0){
        ...
        opcode = OP_PRINT;
    }else if(strcmp(operacao, "stats") == 0){
        opcode = OP_STATS;
    } else {
         opcode = -1;
    }
...
int exec_command(char *str,struct rtable_t *client_rtable){
    ...
    switch (opcode) {
      ...
      case OP_GETKEYS:
          keys = rtable_get_keys(client_rtable);
          if(keys == NULL) {
            printf("\n Não há keys\n");
            return -1;
          }
      ...
      case OP_STATS:
          stats_struct = rtable_stats(client_rtable);
          stats_print(stats_struct);
          free(stats_struct);
          break;
